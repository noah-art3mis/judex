...
def extract_stf_data(html_content, incidente_id, processo_id):
    ...
    data['classe'] = extract_classe(soup)
    data['classe_extenso'] = extract_classe_extenso(soup)
    data['nome_processo'] = extract_nome_processo(soup)
    data['tipo_processo'] = extract_tipo_processo(soup)
    data['liminar'] = extract_liminar(soup)
    data['origem'] = extract_origem(soup)
    data['relator'] = extract_relator(soup)
...
def extract_classe(soup):
    """Extract case class (ADI, ADPF, etc.)"""
    try:
...
def extract_classe_extenso(soup):
...
def extract_nome_processo(soup):
...
def extract_tipo_processo(soup):
...
def extract_relator(soup):
    """Extract relator information"""
    try:
        # From the relator field
...
def extract_partes(soup):
    ...
    try:
        ...
        for parte in partes_elements:
            parte_data = {
                'nome': parte.find('div', class_='nome-parte').get_text().strip() if parte.find('div', class_='nome-parte') else '',
                'tipo': parte.find('div', class_='tipo-parte').get_text().strip() if parte.find('div', class_='tipo-parte') else '',
                'advogados': [adv.get_text().strip() for adv in parte.find_all('div', class_='advogado')]
            }
...
def extract_andamentos(soup):
    ...
    try:
        ...
        for andamento in andamentos_elements:
            andamento_data = {
                'data': andamento.find('div', class_='data-andamento').get_text().strip() if andamento.find('div', class_='data-andamento') else '',
                'descricao': andamento.find('div', class_='descricao-andamento').get_text().strip() if andamento.find('div', class_='descricao-andamento') else '',
                'tipo': andamento.find('div', class_='tipo-andamento').get_text().strip() if andamento.find('div', class_='tipo-andamento') else ''
            }
...
def extract_decisoes(soup):
    ...
    try:
        ...
        for decisao in decisoes_elements:
            decisao_data = {
                'data': decisao.find('div', class_='data-decisao').get_text().strip() if decisao.find('div', class_='data-decisao') else '',
                'tipo': decisao.find('div', class_='tipo-decisao').get_text().strip() if decisao.find('div', class_='tipo-decisao') else '',
                'relator': decisao.find('div', class_='relator-decisao').get_text().strip() if decisao.find('div', class_='relator-decisao') else '',
                'texto': decisao.find('div', class_='texto-decisao').get_text().strip() if decisao.find('div', class_='texto-decisao') else ''
            }
...
def extract_deslocamentos(soup):
    ...
    try:
        ...
        for deslocamento in deslocamentos_elements:
            deslocamento_data = {
                'data': deslocamento.find('div', class_='data-deslocamento').get_text().strip() if deslocamento.find('div', class_='data-deslocamento') else '',
                'destino': deslocamento.find('div', class_='destino-deslocamento').get_text().strip() if deslocamento.find('div', class_='destino-deslocamento') else '',
                'motivo': deslocamento.find('div', class_='motivo-deslocamento').get_text().strip() if deslocamento.find('div', class_='motivo-deslocamento') else ''
            }
...
def scrape_stf_case(html_content, incidente_id, base_url="https://portal.stf.jus.br"):
    ...
    try:
        # Parse main HTML
        case_data = extract_stf_data(html_content, incidente_id)
